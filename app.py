...
=======
st.markdown(
    "Create and calculate color harmonies locked strictly to the **512 colors (9-bit VDP RGB)** of the original hardware.")
>>>>>>> ca4aa52 (Update app.py)
...
# 3-bit sliders for exact color selection
...
# Map the 0-7 indexes directly to the actual VDP hardware voltage steps
...
with col_wheel:
    ...
    for a in angles_bg:
        for r_g in radii_bg:
            # Map wheel brightness dynamically using the base color value
            ...
            # Correlate scatter size with radii to avoid overlapping in the center
            scatter_size = 15 + (r_g * 45)
            ax.scatter(a, r_g, color=f"#{q_r:02X}{q_g:02X}{q_b:02X}", s=scatter_size, alpha=0.9)
    ...
    for idx, color in enumerate(palette):
        ...
        ax.scatter(
            ...
            edgecolor=node_border, s=220, zorder=10
        )
...
st.caption(
    "Sega Genesis / Mega Drive Color Wheel | Conceptualized & Tested by Rodrigo Fontanella | Code co-generated via AI Assist | Open-source community tool.")
